...
- 添加 `scripts/import_fund_trace.py`，支持从用户指定的 `fund-trace.db` 复用导入已有 SQLite 数据。
...
- 选定第一版界面风格为“研究终端 / institutional lab”：深色左侧导航、浅灰工作区、紧凑表格、蓝绿为主操作与收益强调、金色用于黄金/防守资产提示。
- 新增共享工作台组件：`PageHeader`、`Toolbar`、`KpiCard`、`StatGrid`，统一页面标题、筛选区和关键指标展示。
- 实现基金池页面：展示、添加基金。
- 实现实验组页面：创建实验组、查看实验列表。
- 实现实验详情页面：创建实验（含多基金仓位配置）、运行回测。
- 实现回测看板页面：展示所有回测结果对比、查看 Markdown 研究报告。
- 实现压力区间页面：添加和管理压力区间。
- 优化基金池、实验组、实验详情、回测看板、压力区间 5 个页面的结构、统计摘要、表格密度和错误展示。
- 优化实验详情和回测看板的数据请求逻辑，减少串行请求等待。
- 基金池创建、编辑、删除失败时展示后端错误详情；删除确认文案提示引用约束。
- TypeScript 类型检查通过，Vite 构建成功。
- 已通过本地浏览器检查基金池和回测看板主路径；基金池搜索筛选可用，页面布局和核心表格渲染正常。
...
- 前端依赖已安装，构建通过，并已在本地 dev server 做过基础浏览器级验证。
- 当前环境未额外安装 Playwright 依赖，移动端布局暂按响应式 CSS 和构建检查验证，后续可补浏览器矩阵测试。
...
- 补充前端浏览器矩阵测试，覆盖基金弹窗、实验创建、回测报告弹窗和移动端窄屏。
